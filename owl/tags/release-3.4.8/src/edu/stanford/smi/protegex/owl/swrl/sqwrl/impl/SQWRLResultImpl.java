...
/**
 * This class implements the interfaces SQWRLResult and SQWRLResultGenerator. It can be used to generate a result structure and populate it with data; it can
 * also be used to retrieve those data from the result.
 * <p>
 * 
 * This class operates in three phases:
 * <p>
 * 
 * (1) Configuration Phase: In this phase the structure of the result is defined. This phase opened by a call to the configure() method (which will also clear
 * any existing data). In this phase the columns are defined; aggregation or ordering is also specified in this phase. This phase is closed by a call to the
 * configured() method.
 * <p>
 * 
 * (2) Preparation Phase: In this phase data are added to the result. This phase is implicitly opened by the call to the configured() method. It is closed by a
 * call to the prepared() method.
 * <p>
 * 
 ...
 * <p>
 * 
 * SQWRLResultImpl result = new SQWRLResultImpl("TestResult");
 * <p>
 * 
 ...
 * result.openRow();
 * <p>
 * result.addRowData(new IndividualValue("Fred"));
 * <p>
 * result.addRowData(new DataValue(27));
 * <p>
 * result.closeRow();
 * <p>
 * <p>
 ...
 * result.openRow();
 * <p>
 * result.adRowdData(new IndividualValue("Joe"));
 * <p>
 * result.addRowData(new DataValue(34));
 * <p>
 * result.closeRow();
 * <p>
 * <p>
 ...
 * result.openRow();
 * <p>
 * result.addRowData(new IndividualValue("Joe"));
 * <p>
 * result.addRowData(new DataValue(21));
 * <p>
 * result.closeRow();
 * <p>
 * <p>
 ...
 * <p>
 * 
 * The result is now available for reading. The interface SQWRLResult defines the assessor methods. A row consists of a list of objects defined by the interface
 * SQWRLResultValue. There are four possible types of values (1) DataValue, representing literals; (2) IndividualValue, representing OWL individuals; (3)
 * ClassValue, representing OWL classes; and (4) PropertyValue, representing OWL properties.
 * <p>
 * <p>
 * 
 * while (result.hasNext()) {
 * <p>
 * IndividualValue nameValue = result.getIndividualValue("name");
 * <p>
 * DataValue averageValue = result.getDataValue("average");
 * <p>
 ...
 * <p>
 * 
 * There is also a convenience method addRow, which takes a list of SQWRLResultValues. This method automatically does a row open and close. It is expecting the
 * exact same number of list elements as there are columns in the result.
 * <p>
 */
public class SQWRLResultImpl implements SQWRLResult, SQWRLResultGenerator, Serializable
{
	...
	public void addOrderByColumn(int orderedColumnIndex, boolean ascending) throws SQWRLException
	{
		...
		if (isOrdered && (isAscending != ascending)) {
			if (isAscending)
				...
				throw new SQWRLException("attempt to order column " + allColumnNames.get(orderedColumnIndex) + " descending when ascending was previously selected");
		}
	...
	public String getColumnName(int columnIndex) throws SQWRLException
	{
		...
		if (columnIndex < columnDisplayNames.size())
			return columnDisplayNames.get(columnIndex);
		else
	...
	public void addRow(List<SQWRLResultValue> resultValues) throws SQWRLException
	{
		...
		openRow();
		for (SQWRLResultValue value : resultValues)
			addRowData(value);
		closeRow();
	}
	...
	public void addRowData(SQWRLResultValue value) throws SQWRLException
	{
		...
		if (rowDataColumnIndex == getNumberOfColumns())
		...
		if (aggregateColumnIndexes.containsKey(Integer.valueOf(rowDataColumnIndex))
				...
				&& (!aggregateColumnIndexes.get(Integer.valueOf(rowDataColumnIndex)).equals(SQWRLNames.CountDistinctAggregateFunction)) && (!isNumericValue(value)))
	...
	public void prepared() throws SQWRLException
	{
		...
		if (hasAggregates)
			rows = aggregate(rows, allColumnNames, aggregateColumnIndexes); // Aggregation implies killing duplicate rows
		else if (isDistinct)
			rows = distinct(rows);

		if (isOrdered)
			rows = orderBy(rows, allColumnNames, orderByColumnIndexes, isAscending);
	...
	public void next() throws SQWRLException
	{
		...
		if (rowIndex != -1 && rowIndex < getNumberOfRows())
			rowIndex++;
	}
		...
		if (hasLimit()) {
			...
			if (hasNth()) {
				if (nth < 1)
					nth = 1;
				if (nth <= rows.size())
					processedRows.add(rows.get(nth - 1));
				hasSelection = true;
			}
			...
			if (hasNotNth()) {
				...
				if (nth <= rows.size()) {
					...
					processedRows.addAll(rows); // Add everything
				hasSelection = true;
			} // if
			...
			if (hasFirstSelection()) {
				if (firstN < 1)
					firstN = 1;
				if (firstN <= rows.size())
					processedRows.addAll(rows.subList(0, firstN));
				hasSelection = true;
			}
			...
			if (hasNotFirstSelection()) {
				if (firstN < 1)
					firstN = 1;
				if (firstN <= rows.size())
					processedRows.addAll(rows.subList(firstN, rows.size()));
				else
					processedRows.addAll(rows); // Add everything
				hasSelection = true;
			} // if
			...
			if (hasLastSelection()) {
				if (lastN < 1)
					lastN = 1;
				if (lastN <= rows.size())
					processedRows.addAll(rows.subList(rows.size() - lastN, rows.size()));
				hasSelection = true;
			}
			...
			if (hasNotLastSelection()) {
				if (lastN < 1)
					lastN = 1;
				if (lastN <= rows.size())
					processedRows.addAll(rows.subList(0, rows.size() - lastN));
				else
					processedRows.addAll(rows); // Add everything
				hasSelection = true;
			}
			...
			if (hasNotNthSliceSelection()) {
				...
				if (firstN <= rows.size()) {
					...
					if (finish <= rows.size())
						processedRows.addAll(rows.subList(finish, rows.size()));
				} else
					processedRows.addAll(rows); // Add everything
				hasSelection = true;
			} // if
			...
			if (hasNthLastSliceSelection()) {
				...
				if (lastN <= rows.size()) {
					processedRows.addAll(rows.subList(lastN, finish));
				}
				hasSelection = true;
			} // if
	...
	{
		return !hasNotNth() && nth != -1;
	}
	private boolean hasNotNth()
	{
		return notNthSelection;
	}
	...
	private void prepareColumnVectors() throws SQWRLException
	{
		...
		if (getNumberOfColumns() > 0) {
			...
			for (int c = 0; c < getNumberOfColumns(); c++)
				columnVectorMap.put(getColumnName(c), columns.get(c));
		}
	}
	...
	public String toString()
	{
		String result = "[numberOfColumns: " + numberOfColumns + ", isConfigured: " + isConfigured + ", isPrepared: " + isPrepared + ", isRowOpen: " + isRowOpen
				+ ", isOrdered: " + isOrdered + ", isAscending " + isAscending + ", isDistinct: " + isDistinct + ", hasAggregates: " + hasAggregates + "]\n";

		result += "[columnDisplayNames: ";
		for (String columnDisplayName : columnDisplayNames)
	...
	private void initialize()
	{
		...
		numberOfColumns = 0;
		isOrdered = isAscending = isDistinct = false;
	...
	private void checkColumnName(String columnName) throws InvalidColumnNameException
	{
		if (!allColumnNames.contains(columnName))
			throw new InvalidColumnNameException("invalid column name " + columnName);
	} // checkColumnName
	...
	private List<List<SQWRLResultValue>> distinct(List<List<SQWRLResultValue>> rows)
	{
		...
		for (List<SQWRLResultValue> row : localRows)
			if (Collections.binarySearch(result, row, rowComparator) < 0)
				result.add(row);

		return result;
	} // distinct

	private List<List<SQWRLResultValue>> aggregate(List<List<SQWRLResultValue>> rows, List<String> allColumnNames, Map<Integer, String> aggregateColumnIndexes)
			throws SQWRLException
	{
		...
		for (List<SQWRLResultValue> row : result) {
			...
			for (Integer aggregateColumnIndex : aggregateColumnIndexes.keySet()) {
				...
				if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.MinAggregateFunction))
					value = min(values);
				else if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.MaxAggregateFunction))
					value = max(values);
				else if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.SumAggregateFunction))
					value = sum(values);
				else if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.AvgAggregateFunction))
					value = avg(values);
				else if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.CountAggregateFunction))
					value = count(values);
				else if (aggregateFunctionName.equalsIgnoreCase(SQWRLNames.CountDistinctAggregateFunction))
					value = countDistinct(values);
				else
					throw new InvalidAggregateFunctionNameException("invalid aggregate function " + aggregateFunctionName);
	...
	private List<List<SQWRLResultValue>> orderBy(List<List<SQWRLResultValue>> rows, List<String> allColumnNames, List<Integer> orderByColumnIndexes,
	...
	private DataValue min(List<SQWRLResultValue> values) throws SQWRLException
	{
		...
		if (values.isEmpty())
			throw new SQWRLException("empty aggregate list for " + SQWRLNames.MinAggregateFunction);
	...
	private DataValue max(List<SQWRLResultValue> values) throws SQWRLException
	{
		...
		if (values.isEmpty())
			throw new SQWRLException("empty aggregate list for " + SQWRLNames.MaxAggregateFunction);
	...
	private DataValue sum(List<SQWRLResultValue> values) throws SQWRLException
	{
		...
		if (values.isEmpty())
			throw new SQWRLException("empty aggregate list for " + SQWRLNames.SumAggregateFunction);
	...
	private DataValue avg(List<SQWRLResultValue> values) throws SQWRLException
	{
		...
		if (values.isEmpty())
			throw new SQWRLException("empty aggregate list for function " + SQWRLNames.AvgAggregateFunction);
	...
	private static class RowComparator implements Comparator<List<SQWRLResultValue>>
	{
		...
		public RowComparator(List<String> allColumnNames, boolean ascending)
		{
			...
			for (String columnName : allColumnNames)
				orderByColumnIndexes.add(allColumnNames.indexOf(columnName));
		} // RowComparator
