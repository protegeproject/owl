...
public class JavaCodeGenerator {
    ...
    private void printFactoryClass(PrintWriter printWriter) {
        ...
        for (Iterator it = owlModel.getUserDefinedRDFSNamedClasses().iterator(); it.hasNext();) {
            ...
            if (!cls.getNamespace().equals(SWRLNames.SWRL_NAMESPACE)) {  
	            ...
	            printWriter.println("        return new " + implementationName + "(owlModel, new FrameID(OWLUtil.getInternalFullName(owlModel, name)));");
	            printWriter.println("    }");
    ...
    public void createAbstractIndividualClass() throws IOException {
        File file = getImplementationFile("CodeGeneratorIndividual");
        FileWriter fileWriter = new FileWriter(file);
        PrintWriter printWriter = new PrintWriter(fileWriter);
        printAbstractIndividualClass(printWriter);
        fileWriter.close();
    }

    private void printAbstractIndividualClass(PrintWriter printWriter) {
        if (options.getPackage() != null) {
        ...
        printWriter.println();
        printWriter.println("import edu.stanford.smi.protege.model.FrameID;");
        printWriter.println("import edu.stanford.smi.protege.model.KnowledgeBase;");
        ...
        printWriter.println(" * @version generated on " + new Date());
        printWriter.println(" */");
        printWriter.println("public abstract class CodeGeneratorIndividual extends AbstractCodeGeneratorIndividual {");
        printWriter.println();
        printWriter.println("    private static Map<String, Class> protege2ImplementationMap = new HashMap<String, Class>();");
        printWriter.println("    static {");
        for (Object o : owlModel.getUserDefinedRDFSNamedClasses()) {
            RDFSNamedClass clazz = (RDFSNamedClass) o;
            printWriter.println("        protege2ImplementationMap.put(\"" + clazz.getName() + "\", " + getImplementationName(clazz) + ".class);");
        }
        printWriter.println("    }");
        printWriter.println();
        printWriter.println("    public CodeGeneratorIndividual() {");
        printWriter.println("    }");
        printWriter.println();
        printWriter.println("    public CodeGeneratorIndividual(KnowledgeBase kb, FrameID id) {");
        printWriter.println("        super(kb, id);");
        printWriter.println("    }");
        printWriter.println();
        printWriter.println("    protected Map<String, Class> getProtege2ImplementationMap() {");
        printWriter.println("        return protege2ImplementationMap;");
        printWriter.println("     }");
        printWriter.println("}");
    }
    ...
    private String getImplementationExtendsCode(RDFSNamedClass aClass) {
        ...
        if (base == null) {
            return str + "CodeGeneratorIndividual";
        }
        else {
            return str + base;
        }
