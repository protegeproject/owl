...
public abstract class AbstractSWRLRuleEngineBridge implements SWRLRuleEngineBridge
{
  ...
  public void writeInferredKnowledge2OWL() throws SWRLRuleEngineBridgeException
  {
    ...
    writeCreatedClasses(); // Create any OWL classes generated by built-ins in rules. 
    writeCreatedIndividuals(); // Create any OWL individuals generated by built-ins in rules. 
    writeCreatedRestrictions(); // Create any OWL restrictions generated by built-ins in rules. 
    writeCreatedAxioms(); // Create any OWL axioms generated by built-ins in rules. 
  ...
  public int getNumberOfCreatedAxioms() { return createdAxioms.size(); }
  public int getNumberOfCreatedRestrictions() { return createdRestrictions.size(); }
  ...
  { 
    if (!inferredAxioms.contains(owlPropertyAssertionAxiom)) inferredAxioms.add(owlPropertyAssertionAxiom); 
  ...
  public OWLDatatypePropertyAssertionAxiom createOWLDatatypePropertyAssertionAxiom(OWLDatatypePropertyAssertionAxiom axiom) 
    throws SWRLRuleEngineBridgeException
  {
    if (!createdAxioms.contains(axiom)) createdAxioms.add(axiom);
  ...
  {
    for (OWLClass owlClass: createdClasses.values()) owlClass.write2OWL(owlModel);   
  } // writeCreatedClasses
  ...
  {
    for (OWLIndividual owlIndividual: createdIndividuals.values()) owlIndividual.write2OWL(owlModel);
  } // writeCreatedIndividuals

  /**
   ** Create OWL axioms in model for the axioms created by built-ins during rule execution.
   */
  private void writeCreatedAxioms() throws SWRLRuleEngineBridgeException
  {
    for (OWLAxiom axiom : createdAxioms) axiom.write2OWL(owlModel);
  } // writeCreatedAxioms

  /**
   ** Create OWL restrictions in model for the axioms created by built-ins during rule execution.
   */
  private void writeCreatedRestrictions() throws SWRLRuleEngineBridgeException
  {
    for (OWLRestriction restriction : createdRestrictions) restriction.write2OWL(owlModel);
  } // writeCreatedRestrictions
  ...
  {
    for (OWLAxiom axiom : inferredAxioms) axiom.write2OWL(owlModel);
  } // writeInferredAxioms2OWL
