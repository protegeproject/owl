...
public class SWRLParser {
        ...
        do {
            if (justProcessedAtom) {
                if (inHead) message = "Expecting '" + AND_CHAR + "'";
                    ...
                    message = "Expecting '" + IMP_CHAR + "' or '" + AND_CHAR + "'";
            }
            else {
    ...
    private RDFObject parseDObject()
        ...
        // parsed as xsd:int's.
            ...
            try {
                ...
                if (!parseOnly)
                    parsedEntity = owlModel.createRDFSLiteral(parsedString, owlModel.getXSDint());
            }
            catch (NumberFormatException e1) {
                try {
                    floatValue = Float.parseFloat(parsedString);
                    if (!parseOnly)
                        parsedEntity = owlModel.createRDFSLiteral(parsedString, owlModel.getXSDfloat());
                }
                catch (NumberFormatException e2) {
                    throw new SWRLParseException("Invalid data literal: '" + parsedString + "'.");
                }
            }
        }
    ...
    private RDFResource getIndividual(String name) throws SWRLParseException {
        RDFResource resource = owlModel.getRDFResource(name);
        if (resource != null && resource instanceof OWLIndividual) {
            return resource;
        }
        else {
